...
# Let's loop submissions
session = requests.Session()
session.get(url)

hidden = res['hiddenFields']
hidden['pageHistory'] = '0' # Start at 0
html_resp = html

for step in range(10):
    # Extract current hidden fields from html_resp
    ...
    print(f"Step {step} submitting with pageHistory={pageHist.group(1) if pageHist else None}")
